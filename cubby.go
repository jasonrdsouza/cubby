...
func main() {
	...
	addUserToken := addUserCmd.String("token", "", "security token for this user")
	...
	if len(os.Args) < 2 {
		fmt.Println("Please specify subcommand (serve, adduser, removeuser, get, put, remove)")
		flag.Usage()
		os.Exit(1)
	}
	...
	case "adduser":
		...
		err := cubbyServer.AddUser(*addUserName, *addUserToken, *addUserAdmin)
		if err != nil {
			log.Fatal(err)
		}
	case "removeuser":
