...
func (c *CubbyServer) FetchUser(name string, password string) User {
	...
	if err != nil {
		c.log.Printf("Unable to find user with name: %v. %v", name, err)
		return &AnonymousUser{}
	}

	if user.PasswordMatches(password) {
		c.log.Printf("Found valid user with name: %v", name)
		return &user
	} else {
		c.log.Printf("Invalid credentials specified for user with name: %v", name)
		return &AnonymousUser{}
	}
}
